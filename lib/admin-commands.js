...
export async function handleAdminCommand(message, command, args) {
  ...
  if (!(await hasAdmins())) {
    await reply(
      message,
      'No administrator is configured yet. The deployer must set INITIAL_ADMIN_USER_ID in lib/constants.js, redeploy, then send /start from the configured Telegram account to claim the first admin role.',
    );
    return true;
  }
