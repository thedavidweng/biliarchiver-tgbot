...
export async function handleSourceCallback(callback, jobId, expectedOffset) {
  ...
  if (existing.status === 'expired') {
    await editCallbackMessage(
      callback,
      '⏰ This source job expired after a period of inactivity. Send the source link again to resume archiving the remaining items.',
      { parse_mode: 'HTML' },
    );
    await answerCallback(callback, 'This source job expired.');
    return;
  }
