...
export async function handleSourceCallback(callback, jobId, expectedOffset) {
  ...
  const anyAccepted = queued.results.some((result) => result.accepted);
  if (!anyAccepted) {
    await releaseSourceBatch(jobId, expectedOffset);
    const retryJob = { ...job, nextOffset: expectedOffset };
    await editCallbackMessage(callback, '⚠️ The archiver rejected the batch. Please try again.', {
      parse_mode: 'HTML',
      reply_markup: sourceKeyboard(retryJob),
    });
    await answerCallback(callback, 'The archiver rejected the batch. Please try again.', true);
    return;
  }
